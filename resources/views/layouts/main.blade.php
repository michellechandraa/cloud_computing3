...
<html lang="en">
  ...
  <body class="font-sans bg-black text-white">
    ...
    <div class="flex items-center bg-[#0f172a] text-white rounded-full px-4 py-1 w-full max-w-md">
      <svg class="w-5 h-5 text-gray-400 mr-2" fill="currentColor" viewBox="0 0 24 24">
        <path d="M10 2a8 8 0 105.293 14.293l5.147 5.147a1 1 0 001.414-1.414l-5.147-5.147A8 8 0 0010 2zm0 2a6 6 0 110 12A6 6 0 0110 4z"/>
      </svg>

      <input type="text" placeholder="Search" class="bg-transparent text-sm flex-1 focus:outline-none placeholder-gray-400"/>

      <div x-data="{ open: false, selected: 'Title' }" class="relative ml-2 text-sm">
        <button @click="open = !open" class="flex items-center focus:outline-none">
          <span x-text="selected" class="mr-1"></span>
        ...
        <ul
          ...
          x-transition
          class="absolute right-0 mt-1 w-24 bg-white text-black rounded-md shadow-lg z-10"
        >
          <li><button @click="selected = 'Title'; open = false" class="block w-full text-left px-3 py-1 hover:bg-gray-100">Title</button></li>
          <li><button @click="selected = 'Genre'; open = false" class="block w-full text-left px-3 py-1 hover:bg-gray-100">Genre</button></li>
        </ul>
      </div>
    </div>
    ...
    <div x-data="{ open: false }" class="relative">
      <button @click="open = !open" class="focus:outline-none">
        <img src="/img/avatar.jpg" alt="avatar" class="rounded-full w-8 h-8 object-cover">
      </button>
      ...
      >
        <form id="logout-form" action="http://127.0.0.1:8000/logout" method="POST" class="block">
          @csrf
